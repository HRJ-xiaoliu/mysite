...
from wagtail.search import index  # 引入搜索索引模块
from home.models import GlobalSearchMixin

# 子节点 1：科研成果
class Publication(Orderable, index.Indexed): # 继承 index.Indexed
    ...
    pub_type = models.CharField(max_length=20, choices=[('journal', '期刊论文'), ('conf', '会议文章'), ('patent', '授权专利')], default='journal')
    doi = models.URLField(blank=True, default="", verbose_name="外部链接")
    bibtex_raw = models.TextField(blank=True, null=True, default="", verbose_name="BibTeX 原文")
    abstract = RichTextField(blank=True, null=True, verbose_name="成果摘要")
    
    panels = [
        FieldPanel('title'), FieldPanel('authors'), FieldPanel('venue'), 
        FieldPanel('year'), FieldPanel('pub_type'), FieldPanel('doi'), FieldPanel('abstract'),
    ]

class ResearchProject(Orderable, index.Indexed): # 继承 index.Indexed
    ...
    panels = [
        FieldPanel('title'), FieldPanel('role'), FieldPanel('status'), FieldPanel('description'),
    ]

# 母页面：科研主页
# 注意：确保继承了 Page 基类
class ResearchPage(Page, GlobalSearchMixin): 
